...
#   In the first step we make sure the code is complied (stage="make"). The we generate the input dataset
#   (stage="genInp"). The next step is training (stage="train"). These jobs are put on hold, and are only executed after
#   the "genInp" job has finished. This is done using the "hold_jid" command (the particulars may depend on
#   the batch farm). After training, optimization takes place (stage="optim"), where again, the optimization
#   job is put on hold until all training jobs have finished.
# ---------------------------------------------------------------------------------------------------
...
# regression or classification) or to [binCls_nBins] (for binned classification).
# ---------------------------------------------------------------------------------------------------
subType = 0
if subType == 0:
  cmndBase   = "python scripts/des_rndReg.py --randomRegression"     ; optimType = "optimize" ; nTrainJobs = 150
else:
...
for stage in ["make","genInp","train","optim"]:
  ...
  # ---------------------------------------------------------------------------------------------------
  if stage == "make":
  # I've run the make command manually so no need to run it again.
  ...
  for nJobNow in range(nJobs):
    if stage == "genInp":
	# Error occured if I run this command in Perseus. Therefore, just run it manually (no qsub).
    #  cmnd_annz = cmndBase+" --genInputTrees"                    ; qsub["jobName"] = "genInp"
      cmnd_annz = "echo ========== skip genInputTrees =========="; qsub["jobName"] = "genInp"
    if stage == "train":
    ...
    lines   += ["export PYTHONPATH=/home/ade/annz:$PYTHONPATH"]
    lines   += ["export PATH=/home/ade/anaconda3/bin:$PATH"]
    ...
    # compose the job-submission command (specific syntax for a given batch-farm)
    # ---------------------------------------------------------------------------------------------------
