...
if glob.annz["doGenInputTrees"]:
  ...
  if useWgtKNN:
    ...
    glob.annz["cutInp_wgtKNN"]         = "(emag_g<1.0) && (emag_r<1.0) && (emag_i<1.0) && (emag_z<1.0) && (emag_Y<1.0)"                          # only use objects which have small e_mag
    glob.annz["cutRef_wgtKNN"]         = "(emag_g<1.0) && (emag_r<1.0) && (emag_i<1.0) && (emag_z<1.0) && (emag_Y<1.0)"                          # only use objects which have small e_mag
...
if glob.annz["doTrain"]:
  ...
  for nMLMnow in range(glob.annz["nMLMs"]): 
    ...
    if nMLMnow == 0:
      ...
      glob.annz["userWeights_train"] = "(emag_i<=0.7)*1 + (emag_i>0.7)/emag_i"
      glob.annz["userWeights_valid"] = "(emag_i<=0.7)*1 + (emag_i>0.7)/emag_i"
    else:
...
#    glob.annz["userCuts_valid"]           = "(MAGERR_R < 1)"
  ...
  # --------------------------------------------------------------------------------------------------
  glob.annz["addOutputVars"] = "object_id;mag_g;mag_r;mag_i;mag_z;mag_Y;emag_g;emag_r;emag_i;emag_z;emag_Y;g_r;r_i;i_z;z_Y;eg_r;er_i;ei_z;ez_Y"
